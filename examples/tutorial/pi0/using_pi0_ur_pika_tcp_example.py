#!/usr/bin/env python3  # 使用当前环境中的 python3 解释器执行这个 TCP 模式示例脚本。
...
from checkpoint_utils import resolve_model_checkpoint  # 导入本地 ckpt 解析函数，负责下载或定位模型目录。
...
from lerobot.cameras.pika.configuration_pika import PikaCameraConfig  # 导入 Pika 相机配置类。
from lerobot.cameras.pika.configuration_pika import PikaCameraSource  # 导入 Pika 相机源枚举。
...
TCP_KEYS = ("tcp_x.pos", "tcp_y.pos", "tcp_z.pos", "tcp_rx.pos", "tcp_ry.pos", "tcp_rz.pos")  # 定义 TCP 6 维位姿键名。
...
CAMERA_WINDOW_NAME = "UR Pika Wrist Camera (Sent To PI0 TCP)"  # 前台显示窗口标题，强调这是送给 PI0 的同一张图。
...
def parse_args() -> argparse.Namespace:  # 定义命令行参数解析函数。
    parser = argparse.ArgumentParser(  # 创建命令行参数解析器。
        description=(  # 给脚本写一个用途说明。
            "Run UR+Pika in TCP mode with a fixed startup joint pose and fully open gripper before starting "  # 说明脚本会先完成 joint 初始化。
    ...
    parser.add_argument("--height", type=int, default=480)  # 指定相机图像高度。
    parser.add_argument("--fps", type=int, default=30)  # 指定相机采集帧率。
    parser.add_argument("--warmup-s", type=float, default=2.0)  # 指定相机连接后的预热时间。
    parser.add_argument("--steps", type=int, default=200)  # 指定主循环执行多少步。
    parser.add_argument("--execute", type=bool, default=True)  # 指定是否真的把模型动作发给机器人。
...
def print_state_transition(  # 定义打印“当前状态 / 模型预测 / 机器人实际执行”三组信息的辅助函数。
    ...
    print_named_values("Current robot state:", current_state, [*TCP_KEYS, GRIPPER_KEY])  # 打印当前 TCP 和夹爪状态。
    print_named_values("Predicted next robot state:", predicted_state, [*TCP_KEYS, GRIPPER_KEY])  # 打印模型预测的目标状态。
    print_named_values("Applied state after clipping:", applied_state, [*TCP_KEYS, GRIPPER_KEY])  # 打印机器人侧裁剪后的真实执行目标。
...
def build_camera_config(args: argparse.Namespace) -> dict[str, PikaCameraConfig]:  # 定义构造单路相机配置字典的辅助函数。
    return {  # 返回一份新的相机配置字典，避免 joint 和 tcp 机器人实例复用同一个可变配置对象。
        args.camera_key: PikaCameraConfig(  # 使用命令行指定的相机 key 作为观测字段名。
            source=PikaCameraSource.REALSENSE_COLOR,  # 指定相机源为 Pika 设备上的 RealSense 彩色流。
            width=args.width,  # 把命令行给定的宽度写入相机配置。
            height=args.height,  # 把命令行给定的高度写入相机配置。
            fps=args.fps,  # 把命令行给定的帧率写入相机配置。
            warmup_s=args.warmup_s,  # 把命令行给定的预热时间写入相机配置。
        )  # 完成单路相机配置对象构造。
    }  # 完成相机配置字典构造。


def build_initialization_robot(args: argparse.Namespace) -> URPika:  # 定义构造 joint 初始化机器人对象的辅助函数。
    return URPika(  # 返回一个 joint 控制模式的机器人对象，专门用于启动前固定姿态初始化。
        URPikaConfig(  # 构造机器人配置。
            robot_ip=args.robot_ip,  # 写入 UR 控制器 IP。
            gripper_port=args.gripper_port,  # 写入夹爪串口。
            control_mode="joint",  # 明确使用 joint 控制模式完成初始化。
            calibration_dir=args.calibration_dir,  # 写入校准目录。
            cameras=build_camera_config(args),  # 挂上和后续 TCP 阶段一致的单路腕部相机配置。
        )  # 完成机器人配置对象构造。
    )  # 完成 joint 初始化机器人实例化。


def build_tcp_robot(args: argparse.Namespace) -> URPika:  # 定义构造 TCP policy 机器人对象的辅助函数。
    return URPika(  # 返回一个 TCP 控制模式的机器人对象，供后续 PI0 主循环使用。
        URPikaConfig(  # 构造机器人配置。
            robot_ip=args.robot_ip,  # 指定 UR 控制器 IP。
            gripper_port=args.gripper_port,  # 指定夹爪串口或允许运行时自动解析。
            control_mode="tcp",  # 明确使用 TCP 控制模式。
            calibration_dir=args.calibration_dir,  # 指定校准目录。
            movel_acceleration=0.2,  # 设置 TCP 模式下 movel 的加速度。
            movel_velocity=0.05,  # 设置 TCP 模式下 movel 的速度。
            tcp_max_relative_target={  # 设置 TCP 各维度每一步允许的最大相对变化。
                "tcp_x": 0.005,  # 限制 x 方向每步最大变化为 5 mm。
                "tcp_y": 0.005,  # 限制 y 方向每步最大变化为 5 mm。
                "tcp_z": 0.005,  # 限制 z 方向每步最大变化为 5 mm。
                "tcp_rx": 0.03,  # 限制 rx 方向每步最大变化为 0.03 rad。
                "tcp_ry": 0.03,  # 限制 ry 方向每步最大变化为 0.03 rad。
                "tcp_rz": 0.03,  # 限制 rz 方向每步最大变化为 0.03 rad。
            },  # 完成 TCP 相对变化限幅字典构造。
            gripper_max_relative_target_mm=2.0,  # 限制夹爪每步最大变化为 2 mm。
            cameras=build_camera_config(args),  # 挂上和初始化阶段一致的单路腕部相机配置。
        )  # 完成 TCP 机器人配置对象构造。
    )  # 完成 TCP 机器人实例化。
...
def initialize_robot_pose(  # 定义启动前初始化关节姿态和夹爪开度的函数。
    robot: URPika,  # 第一个参数是当前 joint 模式机器人对象。
    ...
    while True:  # 进入重复下发初始化动作的循环。
        ...
        if is_initialized(observation, robot):  # 如果当前已经满足“关节到位 + 夹爪张开”的联合条件。
            print("Initialization complete. Reconnecting in TCP mode.", flush=True)  # 打印初始化完成提示，并说明下一步会切换到 TCP 模式。
            return  # 结束初始化流程，回到主函数继续启动 TCP policy。

        time.sleep(INIT_COMMAND_PERIOD_S)  # 如果还没到位，就等待下一个初始化周期再重发。


def main() -> int:  # 定义脚本主函数，并约定返回整数退出码。
    args = parse_args()  # 读取并解析所有命令行参数。
    device = torch.device(args.device)  # 把设备字符串转换为 torch.device 对象。
    model_path = resolve_model_checkpoint(args.model_id, args.ckpt_dir)  # 解析模型目录：若需要则先下载到本地。
    ...
    print("Warning: PI0 uses absolute actions by default. In TCP mode this means absolute TCP targets.", flush=True)  # 提醒用户 TCP 模式下动作会被当作绝对 TCP pose。

    init_robot = build_initialization_robot(args)  # 先构造一个 joint 模式机器人对象，用于启动前的固定姿态初始化。
    tcp_robot: URPika | None = None  # 预先定义 TCP 机器人变量，方便 finally 中统一清理。

    try:  # 进入外层 try/finally，确保无论中间哪一步失败都能正确清理资源。
        init_robot.connect()  # 先连接 joint 模式机器人，准备执行初始化动作。
        try:  # 进入初始化阶段的 try/finally，确保初始化机器人总会断开。
            if display_enabled:  # 如果当前环境允许弹窗显示。
                print(f"Displaying camera stream from: {args.camera_key}", flush=True)  # 打印当前前台显示的相机 key。
            else:  # 如果当前环境是无头环境。
                print("Camera display disabled because the environment is headless.", flush=True)  # 打印不显示窗口的原因。

            initialize_robot_pose(init_robot, args.camera_key, display_enabled)  # 在启动 TCP policy 之前先让机器人进入固定 joint 姿态并张开夹爪。
        finally:  # 无论初始化是否成功，都要清理初始化机器人。
            if init_robot.is_connected:  # 如果初始化机器人当前仍然处于连接状态。
                init_robot.disconnect()  # 主动断开初始化机器人、夹爪和相机连接。

        tcp_robot = build_tcp_robot(args)  # 初始化完成后，再构造 TCP 模式机器人对象。
        tcp_robot.connect()  # 连接 TCP 模式机器人、夹爪和相机。

        action_features = hw_to_dataset_features(tcp_robot.action_features, "action")  # 把机器人动作 schema 转成 LeRobot dataset action schema。
        obs_features = hw_to_dataset_features(tcp_robot.observation_features, "observation", use_video=False)  # 把机器人观测 schema 转成 LeRobot dataset observation schema。
        dataset_features = {**action_features, **obs_features}  # 合并动作和观测 schema，形成完整 dataset feature 描述。

        print(f"Robot type: {tcp_robot.robot_type}", flush=True)  # 打印当前机器人类型，例如 ur_pika_tcp。
        ...
        for step in range(args.steps):  # 按命令行指定的步数运行主循环。
            ...
            observation = tcp_robot.get_observation()  # 从真实机器人读取当前 TCP 位姿、夹爪状态和默认相机帧。
            camera_frame = attach_fresh_camera_frame(tcp_robot, observation, args.camera_key)  # 再显式抓一张 fresh frame 并回填到 observation。
            ...
            obs_frame = build_inference_frame(  # 把原始 observation 组装成 policy 推理需要的输入格式。
                ...
                robot_type=tcp_robot.robot_type,  # 传入机器人类型元数据。
            )  # 完成推理输入帧构造。
            ...
            model_input = preprocess(obs_frame)  # 对推理输入帧做 tokenizer、归一化和设备搬运。
            ...
            robot_action = make_robot_action(action, dataset_features)  # 把动作张量还原成具名 TCP 命令字典。

            current_state = {key: float(observation[key]) for key in TCP_KEYS}  # 按固定顺序提取当前 6 个 TCP 状态。
            current_state[GRIPPER_KEY] = float(observation[GRIPPER_KEY])  # 追加当前夹爪状态。
            predicted_state = {key: float(robot_action[key]) for key in [*TCP_KEYS, GRIPPER_KEY]}  # 按固定顺序提取模型预测的目标状态。

            if args.execute:  # 如果命令行显式要求执行真机动作。
                applied_action = tcp_robot.send_action(robot_action)  # 真正把动作发送给机器人，并拿到机器人侧裁剪后的实际动作。
                applied_state = {key: float(applied_action[key]) for key in [*TCP_KEYS, GRIPPER_KEY]}  # 按固定顺序提取机器人真正执行的目标状态。
                print_state_transition(step + 1, current_state, predicted_state, applied_state)  # 把当前状态、模型预测和实际执行统一打印出来。
            else:  # 如果没有显式要求执行真机动作。
                print_named_values("Current robot state:", current_state, [*TCP_KEYS, GRIPPER_KEY])  # 打印当前 TCP 和夹爪状态。
                print_named_values("Predicted next robot state:", predicted_state, [*TCP_KEYS, GRIPPER_KEY])  # 打印模型预测的目标状态。
                print("Dry run only. Pass --execute to actually send this TCP action.", flush=True)  # 提示当前只是 dry-run，不会动机器人。
        ...
        if tcp_robot is not None and tcp_robot.is_connected:  # 如果 TCP 机器人已经实例化且仍处于连接状态。
            tcp_robot.disconnect()  # 主动断开 TCP 机器人、夹爪和相机连接。
        if init_robot.is_connected:  # 如果初始化机器人当前仍然处于连接状态。
            init_robot.disconnect()  # 兜底断开初始化机器人连接。
